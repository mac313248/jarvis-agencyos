...
**CRITICAL: Every claim must have an inline citation.** Use markdown links like `[Title](https://example.com/...)` pulling only from the JSON output. Never invent or guess URLs.

Synthesize a response that:
- Leads with the key answer/finding
- Includes specific facts, names, numbers, dates
- Cites every fact inline as `[Source Title](https://example.com/...)` — do not leave any claim uncited
