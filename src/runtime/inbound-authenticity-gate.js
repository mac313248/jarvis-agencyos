...
//   - Authenticity status is produced ONLY by the trusted verification
//     boundary below. Caller-asserted inbound.verification claims are rejected.
//
...
function hasUntrustedVerificationClaims(value) {
  ...
  return UNTRUSTED_VERIFICATION_CLAIM_KEYS.some((k) => Object.prototype.hasOwnProperty.call(value, k));
}
...
export function verifyInboundSignature({ connector, verification_input = {} } = {}) {
  ...
  // Registered HMAC refs require raw envelope input — never assume trust.
  if (ref.startsWith('authver://hmac/')) {
    const header = verification_input.signature_header;
    if (header === undefined || header === null || String(header).length === 0) {
      return {
        authenticity_status: 'UNKNOWN',
...
export async function evaluateInboundAuthenticityGate(backend, inbound) {
  ...
  // Auth-required: resolve connector ONLY from trusted registry (ignore inbound.connector).
