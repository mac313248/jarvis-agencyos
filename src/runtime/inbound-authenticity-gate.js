...
//
// Security invariants (Codex review):
//   - Connectors are loaded ONLY from the trusted tenant connector registry.
//     Caller-supplied inbound.connector objects are NEVER trusted.
//   - Authenticity verification input comes ONLY from trustedVerification
//     (produced by the verification adapter boundary). inbound.verification
//     on the untrusted payload is explicitly ignored.
//
 ...
 *
 * @param {object} opts
 * @param {object} opts.connector - Trusted connector loaded from registry only.
 * @param {object} [opts.verification] - Trusted adapter verification result input.
 */
 ...
 * @param {object} inbound - Untrusted inbound event payload (connector/verification fields ignored).
 * @param {object} [opts]
 * @param {object|null} [opts.trustedVerification] - Verification adapter result from trusted boundary only.
 */
export async function evaluateInboundAuthenticityGate(backend, inbound, { trustedVerification = null } = {}) {
  ...
  // Auth-required: resolve connector ONLY from trusted tenant registry (fail closed).
  // Caller-supplied inbound.connector is NEVER trusted.
  const connector_id = inbound.connector_id ?? null;

  if (!connector_id || !backend) {
    const evidence = buildGateEvidence({
      source_system,
      provider: source_system,
      connector_id,
  ...
  // Authenticity verification input comes ONLY from the trusted adapter boundary.
  // inbound.verification on the untrusted payload is explicitly ignored.
 ...
 * @param {object} [opts]
 * @param {object|null} [opts.trustedVerification] - Trusted verification adapter result (required for auth-required events).
 * @param {string} [opts.now]
 */
export async function processInboundEvent(backend, inbound, { trustedVerification = null, now = new Date().toISOString() } = {}) {
  assertBusinessWriteAutonomyDisabled();
  await requireTenant(backend);

  const gate = await evaluateInboundAuthenticityGate(backend, inbound, { trustedVerification });
