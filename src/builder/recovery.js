...
export async function reconcileAfterRestart(core, {
  ...
  // Restore in-memory current run pointer from durable active run only.
  // Never invent a new launch.
  if (active.length === 1) {
  ...
  if (
    ...
    try {
      ...
      // Read-only reconcile failure is recorded; do not guess run state.
      provider_status = {
        ok: false,
        error: {
  ...
  const tasks = core.store
    .listTasks()
    .filter((t) => !TERMINAL_TASK_STATUSES.has(t.status));

  const snapshots = tasks.map((t) => buildTaskRecoverySnapshot(core, t));
  const blockedTasks = snapshots.filter((s) => s.status === 'BLOCKED');

  // Fence: stale/cancelled runs never become current after restart.
  for (const task of tasks) {
    for (const run of core.store.listRunsForTask(task.task_id)) {
      if (
