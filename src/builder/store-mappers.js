// Shared row mappers for Builder Core stores (SQLite + PostgreSQL).
// Keep JSON parsing and shape conversion out of the two backends.
...
export function rowToTask(row) {
  ...
  return {
    task_id: row.task_id,
    logical_work_id: row.logical_work_id ?? null,
    intent: row.intent,
    ...
    review_required: Boolean(Number(row.review_required)),
    status: row.status,
...
export function rowToLease(row) {
  ...
  return {
    ...
    factory_run_id: row.factory_run_id ?? null,
