...
export function beginRepairAttempt(
  ...
  if (!isRetryableFailureClass(failure_class)) {
    const updated = core.updateTaskStatus(taskId, TASK_STATUS.BLOCKED);
  ...
  const attempts = countAttempts(core, taskId);
  if (attempts >= policy.max_attempts) {
    const updated = core.updateTaskStatus(taskId, TASK_STATUS.NEEDS_OWNER);
    core.store.appendEvent({
      task_id: taskId,
  ...
  const runtimeMs = elapsedRuntimeMs(core, taskId);
  ...
  // Cost budget: explicitly unsupported/unknown — do not invent spend tracking.
  ...
  if (current && current.task_id === taskId) {
    core.store.updateRun(current.factory_run_id, {
  ...
  const fresh = core.createRun({
    task_id: taskId,
    provider,
  });
  core._currentFactoryRunId = fresh.factory_run_id;
  if (task.status !== TASK_STATUS.RUNNING) {
    core.updateTaskStatus(taskId, TASK_STATUS.RUNNING);
  }
  core.store.appendEvent({
    task_id: taskId,
