...
const SENSITIVE_KEY_RE =
  /^(?:CURSOR_API_KEY|CODEX_API_KEY|OPENAI_API_KEY|GITHUB_TOKEN|GH_TOKEN|DATABASE_URL|GHL_.*|HIGHLEVEL_.*|META_.*|FACEBOOK_.*|STRIPE_.*|PAYMENT_.*|PAYPAL_.*|TWILIO_.*|.*(?:_KEY|_TOKEN|_SECRET|_PASSWORD|_PASSWD|_CREDENTIALS?|DATABASE_URL))$/i;
...
export function redactString(text, extraSecrets = []) {
  ...
  out = out.replace(/:\/\/([^:/@]+):([^@/?#]+)@/g, `://$1:${REDACTED}@`);
  // Key=value and JSON "key":"value" forms for common secret names.
  out = out.replace(
    /((?:CURSOR_API_KEY|CODEX_API_KEY|OPENAI_API_KEY|GITHUB_TOKEN|GH_TOKEN|DATABASE_URL|[A-Z0-9_]+(?:_KEY|_TOKEN|_SECRET|_PASSWORD|DATABASE_URL))\s*[=:]\s*)(["']?)([^"',\s}]+)(\2)/gi,
    `$1$2${REDACTED}$4`
  );
  out = out.replace(
    /("(?:CURSOR_API_KEY|CODEX_API_KEY|OPENAI_API_KEY|GITHUB_TOKEN|GH_TOKEN|DATABASE_URL|[A-Za-z0-9_]+(?:_KEY|_TOKEN|_SECRET|_PASSWORD|DATABASE_URL))"\s*:\s*)(")(?:\\.|[^"\\])*(")/gi,
    `$1$2${REDACTED}$3`
  );
  return out;
}
