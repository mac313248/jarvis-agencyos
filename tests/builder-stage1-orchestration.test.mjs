...
describe('Stage-1 live disposable orchestration smoke', () => {
  it('owner command path with real Cursor + verify + Codex when available', async () => {
    ...
    const response = await jarvis.dispatch(JARVIS_COMMANDS.EXECUTE_SOFTWARE_TASK, {
      ...
      review_required: true,
      max_attempts: 2,
      owner_prompt: `Create branch ${branch} if needed. Add file artifacts/stage1-orch-smoke-${stamp}.txt containing "stage1 orch smoke ${stamp}". Commit and open a DRAFT PR to main titled "[DO NOT MERGE] stage1 orch smoke ${stamp}". Do not merge.`,
      orchestration: {
        githubClient: gh,
