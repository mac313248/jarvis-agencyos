...
//   G) caller-supplied connector object is never trusted (Codex finding 1)
//   H) inbound.verification payload claims are ignored (Codex finding 2)
...
/** Trusted verification adapter result (NOT placed on untrusted inbound payload). */
function trustedVerified() {
  return { signaturePresent: true, signatureValid: true };
}
...
describe('inbound authenticity gate — explicit gate before materialization', () => {
  test('A. verified inbound event is accepted and materializes canonical state', async () => {
    ...
    const result = await asRuntimeTenant(db, 'app_runtime', A, async (tx) =>
      processInboundEvent(tx, inbound, { trustedVerification: trustedVerified() }));
  ...
  test('B. unsigned/missing signature is rejected (UNKNOWN)', async () => {
    const inbound = baseInbound({ dedupe_key: 'dedupe-missing-' + randomUUID() });
    const result = await asRuntimeTenant(db, 'app_runtime', A, async (tx) =>
      processInboundEvent(tx, inbound, {
        trustedVerification: { signaturePresent: false, signatureValid: false },
      }));
  ...
  test('C. failed signature verification is rejected (FAILED)', async () => {
    const inbound = baseInbound({ dedupe_key: 'dedupe-failed-' + randomUUID() });
    const result = await asRuntimeTenant(db, 'app_runtime', A, async (tx) =>
      processInboundEvent(tx, inbound, {
        trustedVerification: { signaturePresent: true, signatureValid: false },
      }));
  ...
  test('D. unknown source / unregistered connector is rejected', async () => {
    const gateOnly = await asRuntimeTenant(db, 'app_runtime', A, async (tx) =>
      ...
      }, { trustedVerification: trustedVerified() }));
  ...
  test('E. rejected events never reach canonical state materialization', async () => {
    const cases = [
      {
        inbound: baseInbound({ dedupe_key: 'dedupe-e-missing-' + randomUUID() }),
        trustedVerification: { signaturePresent: false, signatureValid: false },
      },
      {
        inbound: baseInbound({ dedupe_key: 'dedupe-e-failed-' + randomUUID() }),
        trustedVerification: { signaturePresent: true, signatureValid: false },
      },
      {
        inbound: baseInbound({
    ...
    for (const { inbound, trustedVerification } of cases) {
      const result = await asRuntimeTenant(db, 'app_runtime', A, async (tx) =>
        processInboundEvent(tx, inbound, { trustedVerification }));
...
describe('inbound authenticity gate — Codex review hardening', () => {
  test('G. caller-supplied connector object is never trusted (finding 1)', async () => {
    // Attacker supplies a fake active connector with a fake auth ref and valid-looking
    // verification, but points connector_id at a non-existent registry entry.
    const inbound = baseInbound({
      connector_id: 'conn.does-not-exist',
      connector: baseConnector({
        connector_id: 'conn.attacker-fake',
        authenticity_verification_ref: 'authver://fake/hmac-sha256',
        status: 'active',
      }),
      verification: { signaturePresent: true, signatureValid: true },
      dedupe_key: 'dedupe-attack-connector-' + randomUUID(),
    });
  ...
  test('G2. fake connector with matching connector_id but wrong provider is rejected', async () => {
    const inbound = baseInbound({
      source_system: 'attacker-vendor',
      connector: baseConnector({ provider: 'attacker-vendor', status: 'active' }),
      verification: { signaturePresent: true, signatureValid: true },
      dedupe_key: 'dedupe-attack-provider-' + randomUUID(),
    });
    ...
    assert.equal(result.gate.accepted, false);
    assert.equal(result.materialized, false);
    assert.match(result.gate.rejection_reason, /mismatch/i);
  });

  test('H. inbound.verification payload claims are ignored (finding 2)', async () => {
    // Attacker puts signatureValid=true on untrusted payload but trusted boundary says FAILED.
    const inbound = baseInbound({
      verification: { signaturePresent: true, signatureValid: true },
      dedupe_key: 'dedupe-attack-verification-' + randomUUID(),
    });
  ...
  test('H2. missing trustedVerification fails closed even if payload claims verified', async () => {
    const inbound = baseInbound({
      verification: { signaturePresent: true, signatureValid: true },
      dedupe_key: 'dedupe-attack-no-trusted-' + randomUUID(),
    });
